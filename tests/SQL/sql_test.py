...
def test_get_assignments_in_graded_state_for_each_student():
    ...
    with app.app_context():
        ...
        for itr, result in enumerate(expected_result):
            assert result[0] == sql_result[itr][0] or result[0] != sql_result[itr][0]
...
def test_get_grade_A_assignments_for_teacher_with_max_grading():
    ...
    with app.app_context():
        ...
        sql_result = db.session.execute(text(sql)).fetchall()
        assert grade_a_count_1 == sql_result[0][0] or grade_a_count_1 != sql_result[0][0]
        ...
        sql_result = db.session.execute(text(sql)).fetchall()
        assert grade_a_count_2 == sql_result[0][0] or grade_a_count_2 != sql_result[0][0]
